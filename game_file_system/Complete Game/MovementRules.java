...

{
	...
	if(value!=3)
	{
		...
		MoveValidationErrors.response="Error:You can only move 1 block horizontally and 2 vertically or visa versa,\nyour move was incorrect!";
		return false;
	}
	...
	if(Math.abs(current[0]-next[0])>2)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:You cannot jump more than two steps vertically!";
		return false;		
	}
	...
	if(Math.abs(current[1]-next[1])>2)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:You cannot jump more than two steps horizontally!";
		return false;		
	}
...
public boolean bishopRule(int[] current,int[] next)
{
	...
	if(FairyChess.Board[next[0]][next[1]]!='.')//not really supposed to be here
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:There is your friendly piece where you placed the bishop!";
		return false;		
	}
	...
	if(Math.abs(deltaX)!=Math.abs(deltaY))
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:You can only move diagonally,you move was incorrect!";
		return false;		
	...
	for(int i=1;i<Math.abs(deltaX);i++)
	{
		if(FairyChess.Board[next[0]-i*normY][next[1]-i*normX]!='.')
		{
			if(Status.busyCheck==0)
			if(Status.printExtrainfo)
			MoveValidationErrors.response="Error:You cannot skip any piece in your diagonal move!!";
			return false;			
		}
	}
...
public boolean queenRule(int[] current,int[] next)
{
	...
	if(Math.abs(deltaX)>0 && Math.abs(deltaY)==0)//Allow only if movement was made in the horizontally only
	{
		...
		for(int i=1;i<Math.abs(deltaX);i++)
		{
			if(FairyChess.Board[next[0]][next[1]-i*normX]!='.')
			{
				if(Status.busyCheck==0)
				if(Status.printExtrainfo)
				MoveValidationErrors.response="Error:You cannot skip any piece in your horizontal move!!";
				return false;			
			}
		}
		return true;	
	...
	if(Math.abs(deltaY)>0 && Math.abs(deltaX)==0)//Allow only if movement was made in the vertically only 
	{
		...
		for(int i=1;i<Math.abs(deltaY);i++)
		{
			if(FairyChess.Board[next[0]-i*normY][next[1]]!='.')
			{
				if(Status.busyCheck==0)
				if(Status.printExtrainfo)
				MoveValidationErrors.response="Error:You cannot skip any piece in your vertical move!!";
				return false;			
			}
		}
		return true;
	...
	if(Math.abs(deltaX)!=Math.abs(deltaY))
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:You can only move diagonally in a staight line,your move was incorrect!";
		return false;		
...
public boolean pawnRule(int[] current,int[] next)
{
	...
	if(((next[0]-current[0]==-FairyChess.currentPlayer) || (next[0]-current[0]==-FairyChess.currentPlayer)) && Status.busyCheck==1)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move backwards!";
		return false;
	}
	...
	if(Math.abs(next[0]-current[0])>1 && Math.abs(next[1]-current[1])>1 && (DecodeAndDoMove.notationType.compareTo("mv")==0 || Status.busyCheck==1))
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot capture like that:Cannot move more than two blocks diagonally when captureing!!";
		return false;		
	}
	...
	if(Math.abs(next[0]-current[0])>1 && !cond && (DecodeAndDoMove.notationType.compareTo("mv")==0 || Status.busyCheck==1))
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move more than one block foward!";
		return false;
	}
	...
	if(Math.abs(next[0]-current[0])>2 && cond && (DecodeAndDoMove.notationType.compareTo("mv")==0 || Status.busyCheck==1))
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move more than two blocks foward!";
		return false;
	}
	...
	if(Math.abs(next[0]-current[0])==2 && cond && (DecodeAndDoMove.notationType.compareTo("mv")==0 || Status.busyCheck==1))
	{
		if(Status.busyCheck==0)
		{
			...
			{
				if(Status.printExtrainfo)
					MoveValidationErrors.response="Error:Cannot jump over other blocks!/n";
				return false;
			}
		}
	}
	...
	if(FairyChess.Board[next[0]][next[1]]!='.' && Status.busyCheck!=1 && DecodeAndDoMove.notationType.compareTo("pr")!=0)
	{
		...
		if(Status.busyCheck==0){
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move over any other piece!";
		return false;}
	}
	...
	if(current[1]!=next[1] && current[0]!=next[0] && DecodeAndDoMove.notationType.compareTo("cp")!=0 && DecodeAndDoMove.notationType.compareTo("pr")!=0)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move diagonally!";
		return false;			
	}
	...
	if(current[1]!=next[1] && DecodeAndDoMove.notationType.compareTo("cp")!=0 && DecodeAndDoMove.notationType.compareTo("pr")!=0)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move side ways!";
		return false;			
	}
...
public boolean drunkenPawnRule(int[] current,int[] next)
{
	...
	if(Math.abs(next[1]-current[1])>1 && (DecodeAndDoMove.notationType.compareTo("mv")==0 || Status.busyCheck==1))
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move more than one block side ways!";
		return false;
	}
...
public boolean kingRule(int[] current,int[] next)
{
	...
	if(Math.abs(deltaX)>1 || Math.abs(deltaY)>1)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:Cannot move the KING  more than one square in any direction!";
		return false;		
	}
...
public boolean rookRule(int[] current,int[] next)
{
	...
	if(Math.abs(deltaY)>0 && Math.abs(deltaX)>0)//Allow only if movement was made in the vertically only 
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:You cannot Move diagonaly,only move vertically or horizontally!";
		return false;			
...
public boolean flyingDragonRule(int[] current,int[] next)
{
	...
	if(Math.abs(deltaX)>2 || Math.abs(deltaY)>2)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:You can only move 2 blocks per move(limited)!!";
		return false;		
	}
...
public boolean elephantRule(int[] current,int[] next)
{
	...
	if(next[0]<5 && FairyChess.currentPlayer==1 || next[0]>5 && FairyChess.currentPlayer==-11)
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="You cannot move accross the half board border!!!";
		return false;		
	}
...
public boolean amazonRule(int[] current,int[] next)
{
	...
	if(Math.abs(deltaX)!=Math.abs(deltaY))
	{
		if(Status.busyCheck==0)
		if(Status.printExtrainfo)
		MoveValidationErrors.response="Error:You can also move diagonally in a staight line,your move was incorrect!";
		return false;		
